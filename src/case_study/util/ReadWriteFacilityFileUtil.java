...
public class ReadWriteFacilityFileUtil {
    ...
    public static Map<Facility, Integer> readFacilityFile(String path) {
        ...
        for (String line : stringList) {
            info = line.split(",");
            if (info.length == 10) {
                facilityList.put(new Villa(info[0], info[1], Double.parseDouble(info[2]), Integer.parseInt(info[3]),
                        Integer.parseInt(info[4]), info[5], info[6], Double.parseDouble(info[7]),
                        Integer.parseInt(info[8])), Integer.parseInt(info[9]));
            } else if (info.length == 8) {
                facilityList.put(new Room(info[0], info[1], Double.parseDouble(info[2]), Integer.parseInt(info[3]),
    ...
    public static void writeFacilityFile(String path, Map<Facility, Integer> facilityList) {
        ...
        for (Facility key : keys) {
            data.append(key.getInfor()).append(",").append(facilityList.get(key)).append("\n");
        }
